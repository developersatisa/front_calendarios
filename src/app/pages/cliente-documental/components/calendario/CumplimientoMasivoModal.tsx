...
import { getClienteProcesoHitosPorFecha, cumplimientoMasivo, ClienteProcesoHitoResumido } from '../../../../api/clienteProcesoHitos'
...
const CumplimientoMasivoModal: FC<Props> = ({ show, onHide, onSuccess, onError }) => {
    ...
    const [fechaCumplimiento, setFechaCumplimiento] = useState(new Date().toISOString().split('T')[0])
    const [horaCumplimiento, setHoraCumplimiento] = useState(new Date().toTimeString().slice(0, 5))
    const [observacion, setObservacion] = useState('')

    // Resetear al abrir
    // Resetear y cargar al abrir
    useEffect(() => {
        if (show) {
            const now = new Date()
            ...
            setMes(currentMes)
            setAnio(currentAnio)
            setFechaCumplimiento(now.toISOString().split('T')[0])
            ...
            setHasSearched(true)
            loadData(currentAnio, currentMes, 1, limit, sortField, sortDirection)
        }
    }, [show])
    ...
    const handleBuscar = () => {
        ...
        loadData(anio, mes, 1, limit, sortField, sortDirection)
    }
    ...
    const handleSubmit = async () => {
        if (selectedIds.length === 0) {
            onError('Debes seleccionar al menos un registro')
            return
        }

        setLoading(true)
        try {
            await cumplimientoMasivo({
                ids: selectedIds,
            ...
            onSuccess(`Se han cumplimentado ${selectedIds.length} hitos correctamente`)
            onHide()
        } catch (error) {
            console.error(error)
            onError('Error al realizar el cumplimiento masivo')
        } finally {
            setLoading(false)
        }
    }
            ...
            <Modal.Body style={{ backgroundColor: 'white', padding: '24px' }}>
                ...
                >
                    <div className="row g-3 align-items-end">
                        ...
                        <div className="col-md-3">
                            ...
                            >
                                {loadingData ? 'Buscando...' : <><i className="bi bi-search me-2 text-white"></i>Buscar</>}
                            </button>
                        </div>
                    </div>
                </div>
                ...
                >
                    ...
                    >
                        ...
                        <tbody>
                            {items.length === 0 ? (
                                <tr>
                                    <td colSpan={4} style={{ textAlign: 'center', padding: '40px 20px', color: atisaStyles.colors.dark }}>
                ...
                <div
                    style={{
                        ...
                        backgroundColor: '#f8f9fa',
                        borderRadius: '12px',
                        border: `1px solid ${selectedIds.length > 0 ? atisaStyles.colors.accent : atisaStyles.colors.light}`,
                        padding: '24px',
                ...
                >
                    <div className="d-flex align-items-center mb-4">
                        <i className="bi bi-pencil-square me-2" style={{ fontSize: '20px', color: atisaStyles.colors.primary }}></i>
                        <h6 className="mb-0 fw-bold" style={{ color: atisaStyles.colors.primary }}>Datos para Cumplimiento</h6>
                    </div>

                    <div className="row g-3">
                        <div className="col-md-3">
                            <label className="form-label required fs-7 fw-bold" style={{ color: atisaStyles.colors.dark }}>Fecha</label>
                            <input
                                type="date"
                                className="form-control form-control-solid"
                        ...
                        <div className="col-md-3">
                            <label className="form-label required fs-7 fw-bold" style={{ color: atisaStyles.colors.dark }}>Hora</label>
                            <input
                                type="time"
                                className="form-control form-control-solid"
                        ...
                        <div className="col-md-6">
                            <label className="form-label fs-7 fw-bold" style={{ color: atisaStyles.colors.dark }}>Observaciones</label>
                            <input
                                type="text"
                                className="form-control form-control-solid"
                                value={observacion}
